...
def geny(n, m):
    def f(x1, x2, x3):
        f = 8.2 + 4.4*x1 + 0.8*x2 + 1.2*x3
        f += 7.4*x1*x1 + 0.2*x2*x2 + 8.3*x3*x3
        f += 1.1*x1*x2 + 0.1*x1*x3 + 4.7*x2*x3 + 9.8*x1*x2*x3
        return f
    
    mat_y = [[round(f(*xnat[i]) + random.randint(0, 10) - 5, 2) for j in range(m)]for i in range(n)]
    for elem in mat_y:
...
def get_b(lmaty):
    ...
    a = numpy.array([[*a0], [*a1], [*a2], [*a3], [*a4], [*a5], [*a6], [*a7], [*a8], [*a9], [*a10]])
    c0 = [calcxi(n, [lmaty])]
...
def all_print():
    ...
    for i in range(n):
        ...
        print("{:^ 11}{:^ 11}"
              .format(maty[i][-1],
                      round(sum([cmb(xnat[i])[j] * b0[j] * dmas[j] for j in range(11)])), 2), end="")
